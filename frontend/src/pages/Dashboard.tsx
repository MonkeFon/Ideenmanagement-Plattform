...
function MyTasksCard() {
  ...
  function roleBadges(i: Idea) {
    const roles: string[] = []
    if (i.assignedReviewerId === user.id || i.preferredReviewerId === user.id) roles.push('Prüfer')
    if (i.assignedManagerId === user.id || i.preferredManagerId === user.id) roles.push('Ideenmanager')
    return roles.join(' · ')
  }
